 ...
 */
class SQL implements \Evoke\Iface\DB
{
   ...
   public function __construct($setup=array())
   {
      ...
      if (!$this->setup['DB'] instanceof \Evoke\Iface\DB)
      {
   ...
   public function beginTransaction()
   {
      ...
      {
	 throw new \Evoke\Exception_DB(__METHOD__, 'Already in a transaction.');
      }
      else
      {
   ...
   public function commit()
   {
	 ...
	 return $this->setup['DB']->commit();
      ...
      {
	 throw new \Evoke\Exception_DB(__METHOD__, 'Not in a transaction.');
      }
   }
   ...
   public function prepare($statement, $driverOptions=array())
   {
	 ...
	 $this->setAttribute(
	    ...
	    array('\Evoke\DB\PDOStatement_Extended', array($namedPlaceholders)));
	 
	 return $this->setup['DB']->prepare($statement, $driverOptions);
      }
      catch (\Exception $e)
      {
	 throw new \Evoke\Exception_DB(__METHOD__, '', $this->setup['DB'], $e);
      }
   }
   ...
   public function update($tables, $setValues, $conditions='', $limit=0)
   {
	 ...
	 $stmt = $this->prepare($q);
      ...
      {
	 throw new \Evoke\Exception_DB(__METHOD__, 'Prepare', $this->setup['DB'], $e);
      }
      ...
      {
	 throw new \Evoke\Exception_DB(__METHOD__, 'Execute', $stmt);
      }
   ...
   private function expand($arg, $separator=',')
   {
	 ...
	 else
	 {
      ...
      {
	 throw new \Evoke\Exception_Base(
	    __METHOD__,
